    ...
    export default {
        ...
        computed: {
            cssClasses: function () {
                ...
                if (this.variant) {
                    classes[`custom-scrollbar--${this.variant}`] = true;
                }

                if (this.size) {
                    classes[`custom-scrollbar--${this.size}`] = true;
                }

                return classes;
            },
            text: function () {
                var wordCount = parseInt(this.wordCount || 0);
                var wordIndex = this.randomInt(0, this.words.length - wordCount - 1);
                return this.words.slice(wordIndex, wordIndex + wordCount).join(' ').replace(/[.,]/g, '');
            }
        },
        methods: {
